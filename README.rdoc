...
Inside the app you can upload a csv files to fill the pool of questions, but this file need to be formatted in the same way that appears at the root of this repository.
...
== Patterns used
* *Model-View-Controller* : The application follows the classical web implementation of the MVC architectural pattern. The models (.rb files) and views (.erb files) are stored in the corresponding models and views directory. The controller are the (.rb files) inside of controllers directory and the app_router.rb
* *Adapter* :
